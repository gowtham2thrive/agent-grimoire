...
description: >-
  Universal technology selection, capability discovery, and build-vs-adopt protocol.
  Use when a project requires a capability (libraries, frameworks, services, tools,
  algorithms, or standards) before adding dependencies or writing custom code.
  Do NOT activate for routine updates of already-adopted packages (use dependency-management)
  or for internal business logic implementations (use code-quality). Enforces the Triviality
  Threshold, Framework-First inspection, the 6-Tier Solution Spectrum (Built-in -> Framework ->
  Compose -> Vendor -> Adopt -> Build), the 10-Axis Candidate Evaluation Matrix,
  the Anti-Corruption Adapter Mandate, and durable Solution Adoption Records (SAR).
---
...
1. **Phase 1 — Framing & Triviality Check**: Define the exact capability required, its invariant success conditions, and operational constraints (latency, memory, concurrency). Apply the **Triviality & Locality Axiom**: if the capability is a pure algorithmic transformation that is easily implemented and verified without external I/O, **bypass discovery and build directly in-place** (see Section 2).
...
3. **Phase 3 — Ecosystem Cartography & Manifest Grounding**: When internal discovery indicates an external capability is justified:
   ...
   - Enforce **Manifest Grounding**: verify candidate package existence in authoritative registries before evaluation to eliminate hallucinated dependencies.
...
5. **Phase 5 — Selection, Adapter Insulation & Decision Recording**:
   - Classify the decision into exactly one tier on the **6-Tier Solution Spectrum** (see Section 3).
   ...
   - Emit a durable **Solution Adoption Record (SAR)** capturing rationale, metrics, and rejected alternatives (see [`references/adoption-record-template.md`](references/adoption-record-template.md)). Hand off installation and lockfile updates to `dependency-management`.
...
| **Build vs. adopt, vendoring, framework reuse, trade-offs** | [`references/solution-spectrum.md`](references/solution-spectrum.md) | The 6-Tier Solution Spectrum, TCO economic balance, custom-build justification, and triviality threshold rules. |
...
| :--- | :--- | :--- | :--- |
| **`micro`** | Pure logic, helper function, localized calculation with no external I/O. | Triviality check. Inspect standard library. If absent, author clean, typed implementation inline. | Direct code implementation (zero external research). |
| **`standard`** | Non-trivial utility, CLI parser, validation schema, format parser. | Local dependency audit $\to$ Framework check $\to$ 2-3 candidate scan $\to$ Manifest grounding $\to$ Rapid 10-axis screen. | Adapter-wrapped adoption or custom build + brief commit justification. |
| **`architectural`** | Database driver, auth/crypto engine, state store, UI component kit, ORM/query layer. | Full 5-phase lifecycle: Workload envelope mapping $\to$ Monorepo audit $\to$ Multi-candidate deep dive $\to$ Transitive tree audit $\to$ Proof-of-concept benchmark. | Formal Solution Adoption Record (`SAR-xxx.md`) + Anti-Corruption Adapter. |
| **`emergency`** | Active CVE mitigation, broken dependency build, abandoned package replacement. | Scoped replacement search strictly filtering for drop-in API compatibility and verified maintenance vitality. | Attributable replacement recommendation handed to `dependency-management`. |

---

## 4 · The 6-Tier Solution Spectrum
...
flowchart TD
    ...
    T5 -- Yes --> S5["Tier 5: External Package Adoption<br/>(Adopt behind Anti-Corruption Adapter)"]
...
2. **Tier 2 — Active Framework Feature**: Use framework primitives (framework middleware, built-in auth, native routing) before adding third-party plugins.
...
4. **Tier 4 — Vendoring / Extraction**: When a large library is needed for a single standalone algorithm, extract the standalone routine under a compatible permissive license with explicit copyright attribution, avoiding massive transitive dependency trees.
5. **Tier 5 — External Package Adoption**: Adopt an established, actively maintained package when domain complexity (e.g., SQLite, Tree-sitter, libsodium, full-featured RFC implementations) makes in-house construction uneconomical. Hand package installation to `dependency-management`.
...
| **3. Vitality & Maintenance** | Is it actively maintained with frequent releases or stable maturity? | Abandoned repository with unaddressed critical bugs or zero response to security advisories. *(Note: Stable, feature-complete libraries with low churn are exempt if no open critical issues exist).* |
...
| **6. Maturity & Quality** | Does it possess automated test suites and production evidence? | Zero test suite in upstream repository; pre-1.0 with breaking churn. |
| **7. Integration Cost & TCO** | What is the transitive dependency weight, bundle size, and glue code? | Pulls excessive transitive dependencies for a non-critical utility. |
| **8. Performance & Resources** | Can it operate within the project's latency, memory, and throughput envelope? | Allocates excessively on hot path or blocks the event loop. |
...
1. **The Left-Pad Anti-Pattern**: Never install a package for trivial string, math, or array manipulation that can be implemented cleanly in a few lines.
...
4. **The Zombie Attraction Anti-Pattern**: Never choose an abandoned package merely because it has high historic vanity metrics if maintenance has ceased and security reports are ignored.
...
1. **Manifest Grounding Certified**: Verify candidate exists in the official registry, author is authentic, and version is pinned.
2. **Transitive Audit Verified**: Perform a dry-run check to confirm clean dependency tree resolution with zero license conflicts.
3. **Anti-Corruption Adapter Specified**: Define the project-owned interface signature before writing glue code.
4. **Solution Adoption Record Emitted**: For `architectural` decisions, record the decision in `docs/adr/` or `SAR-xxx.md` following project convention.
