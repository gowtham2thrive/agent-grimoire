...
name: ux-engineering
description: >-
  Universal user experience (UX) engineering, cognitive ergonomics, and interaction architecture protocol.
  Use when designing, evaluating, auditing, or refactoring user journeys, task flows, information architecture,
  UX content, and usability across any medium (Web, Mobile, Desktop, TUI/CLI, Voice, Canvas,
  or AI/Agent interfaces). Do NOT activate for pure visual styling (use design-philosophy) or
  assistive accessibility compliance (use accessibility). Enforces user goal protection, the 8 Universal UX Invariants,
  HCI empirical laws (Fitts, Hick-Hyman, Miller, Jakob, Tesler, Doherty, Postel), evidence-grounded research (JTBD),
  reversible error recovery, and 0–4 severity usability auditing.
---

# UX Engineering: Universal Cognitive Ergonomics & Interaction Protocol

> **Mandate**: *A visually polished interface is not automatically good UX.* Protect user goals, findability, mental model alignment, predictable feedback, error recovery, and evidence-grounded utility before visual polish. Structure interaction flows through human cognitive physics and state machine determinism; visual polish strictly serves those functional constraints. Never restrict architectural creativity—liberate it through cognitive clarity.
...
flowchart LR
    ...
    P4 --> P5["5. Heuristic & Usability Gate<br/>(Walkthrough, severity 0-4, recovery)"]
```
...
5. **Heuristic & Usability Gate**: Subject the interface to an adversarial cognitive walkthrough against the 8 Universal Invariants and 10 Nielsen Heuristics. Audit interaction operability and error tolerance. Grade defects on the 0–4 Nielsen severity scale (see [`references/usability-auditing-and-metrics.md`](references/usability-auditing-and-metrics.md)).
...
| **User pain points, feature motivation, repo issue analysis** | [`references/evidence-grounded-discovery.md`](references/evidence-grounded-discovery.md) | Mining repo friction, grounded JTBD formulation without hallucinations. |
| **Navigation trees, menus, search, tagging, sitemaps** | [`references/information-architecture.md`](references/information-architecture.md) | Domain modeling, navigation topologies, wayfinding, progressive disclosure. |
| **Forms, wizards, state machines, undo, async actions, AI UX** | [`references/interaction-flows-and-states.md`](references/interaction-flows-and-states.md) | Critical path pruning, state machines (Idle/Optimistic/Undo/Error), AI agent streaming mediation. |
| **Button copy, error text, onboarding copy, tooltips** | [`references/ux-writing-and-microcopy.md`](references/ux-writing-and-microcopy.md) | The 3-part error formula, plain language rules, active voice CTAs, tone consistency. |
| **Audit reviews, PR review of UI, heuristic evaluations** | [`references/usability-auditing-and-metrics.md`](references/usability-auditing-and-metrics.md) | Nielsen 0–4 severity scale, cognitive walkthrough protocol, mathematical finding schema. |
...
| :--- | :--- | :--- | :--- |
| **`micro`** | Button label, localized error message, single input validation, tooltip. | Verify plain language, active voice, actionable error recovery, and hit target bounds. | **Zero preamble essay**. Emit the code/copy diff directly with a 1-line rationale. |
...
| **`system`** | Full application shell, navigation tree overhaul, agentic conversation system, new product module. | Full 5-phase lifecycle; domain ontology; navigation taxonomy; wayfinding; multi-state matrix. | Structured **JTBD $\to$ IA $\to$ Flow Spec** + complete implementation. |
| **`audit`** | Pre-release UX review, heuristic inspection, PR review of user interface. | Adversarial cognitive walkthrough; 10 Nielsen heuristics; 0–4 severity grading; mathematical citation schema. | **Mathematical Finding Matrix** with anchored citations, severity, and code patches. |

### The 3-Line UX Intent Protocol (For `flow` and `system` modes)
When operating in `flow` or `system` mode, summarize UX intent in exactly 3 structured lines before emitting implementation code:
```markdown
> **JTBD**: When [trigger/situation], user wants to [motivation/action], so they can [functional outcome].
> **Task Path**: [Step count: N] · [Primary affordance] · [Feedback: Immediate / Optimistic / Progress]
> **Error Strategy**: [Inline prevention / Non-blocking recovery / Reversible undo]
```

> **Boundary**: This skill owns *user task flows, information architecture, and cognitive load optimization*. For *visual structure and aesthetics* (typography, color, spacing), activate `design-philosophy`. For *inclusive access for disabled users* (screen readers, ARIA, focus stack machine), activate `accessibility`.
...
* **Respect Universal Conventions**: Users spend most of their time on other systems. Never invent idiosyncratic navigation schemes, non-standard gestures, or bizarre iconography when proven conventions exist.
...
* **Immediate Acknowledgment**: User gestures must trigger visible feedback within $\le 100\text{ms}$.
* **Async Operations**: Operations taking $> 400\text{ms}$ must present active progress indicators or optimistic state rendering.
* **Target Reachability (Fitts's Law)**: Frequently accessed actions must have prominent, easily acquired interactive regions.

### 4.4 Invariant 4: Reversible Action & Hostile Error Tolerance (Tesler & Postel)
* **Undo Over Confirmation Modals**: Asking "Are you sure?" causes modal fatigue, leading users to reflexively confirm without reading. Make destructive actions instantly reversible (Undo toast / Trash snapshot) rather than interrogative.
...
### 4.7 Invariant 7: Error Forgiveness & Inline Guidance
* **Prevent Before Reporting**: Prevent errors where possible via inline validation, sensible defaults, and smart input masks.
* **Preserve User Input**: Never clear user-entered form data upon validation failure. Highlight the specific field in error and retain all valid entries.

### 4.8 Invariant 8: Autonomous & Non-Deterministic Interaction Contracts (Modern AI & Agents)
* **Streaming Liveness Perception**: AI-generated responses must begin streaming initial semantic tokens within $\le 200\text{ms}$ to maintain the perception of system liveness.
* **Blast-Radius Mediation**: When an AI agent executes tools that mutate state (file writes, database edits, external API calls), it must present clear pre-flight parameters and offer a rollback/undo mechanism.
...
| Technology Platform | Input & Sensor Modality | Optimistic Feedback | Undo Mechanism |
| :--- | :--- | :--- | :--- |
| **Modern Web** | Pointer click, Enter key, Tap | Optimistic DOM update + muted pulse | Floating Snackbar with "Undo" button (5s timer) |
| **Mobile Touch** | Capacitive tap, Haptic motor | Immediate UI state change + light impact haptic | Bottom toast bar with Undo action |
| **Terminal CLI / TUI** | Keystroke (`d`, `Enter`), Signals | Terminal redraw in $< 16\text{ms}$ | Press `u` within session or auto-backup snapshot |
| **Desktop Application** | Mouse click, Shortcut (`Cmd+Delete`) | Immediate item removal from list | Standard `Cmd+Z` / `Ctrl+Z` menu hook & toast |
| **AI / Agentic Interface** | Natural language prompt, Tool call | Streaming acknowledgment token ($\le 200\text{ms}$) | Rollback step button / edit previous turn |
...
## 8 · The Clean UX Stopping Contract

A UX engineering task is strictly **COMPLETE** only when:
- [ ] User goals are formally grounded in the Job-to-be-Done (JTBD).
- [ ] Navigation paths maintain unambiguous wayfinding (Where am I, How did I get here, What can I do next).
- [ ] Critical interaction paths adhere to feedback latency budgets ($\le 100\text{ms}$ acknowledgment, $\le 400\text{ms}$ progress).
- [ ] Destructive actions provide reversible undo or safe recovery rather than modal fatigue.
- [ ] Error messages satisfy the 3-part invariant (cause, impact, actionable next step).
- [ ] In audit mode, all findings follow the mathematical finding schema with concrete code remediations.
